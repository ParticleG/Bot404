...
def upload_video(session, video_info):
    ...
    try:
        if video_info[4] == 'UCoSrY_IQQVpmIRZ9Xf-y93g':
            ...
            send_message_auto(session,
                              '成功上传视频：“' + video_info[3] + '”到Amelia组盘')
            ...
            send_message_auto(session,
                              '成功上传视频：“' + video_info[3] + '”到Kiara组盘')
        elif video_info[4] == 'UCL_qhgtOy0dy1Agp8vkySQg':
            shutil.copy(f'/usr/go-cqhttp/quin33/caches/{video_info[2]}.mp4',
                        f'/usr/drive/holoen/callio/{new_filename}.mp4')
            send_message_auto(session,
                              '成功上传视频：“' + video_info[3] + '”到Callio组盘')
        else:
            shutil.copy(f'/usr/go-cqhttp/quin33/caches/{video_info[2]}.mp4',
                        f'/usr/drive/holoen/others/{new_filename}.mp4')
            send_message_auto(session,
                              '成功上传视频：“' + video_info[3] + '”到Others盘')
        download_list.pop(session.event.message_type + ':' +
...
def download_video(session, url):
    class Logger(object):
        ...
        def warning(msg):
            print('Warning: ' + msg)
            nonebot.get_bot().sync.send_private_msg(user_id=1135989508,
                                                    message="Warning: " + msg)
