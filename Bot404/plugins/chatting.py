...
@on_command('jrrp', aliases=('rp', '今日人品', '人品'), only_to_me=False, shell_like=True)
async def jrrp(session: CommandSession):
    await session.send(session.get('res'))


@on_command('jrcp', aliases=('cp', '今日CP', 'CP'), only_to_me=False, shell_like=True)
    ...
    if not len(args):
        ...
        if group_member_list[user_couple]['user_id'] == session.event.user_id:
            if session.event.group_id == 901231107:
                session.state['res'] = _cq_at_parser(session.event.user_id) + '你没有今日CP，快和Gura贴贴去吧！'
            elif session.event.group_id == 829619725:
                session.state['res'] = _cq_at_parser(session.event.user_id) + '你没有今日CP，快和Amelia贴贴去吧！'
            elif session.event.group_id == 203690749:
                session.state['res'] = _cq_at_parser(session.event.user_id) + '你没有今日CP，快和Ina贴贴去吧！'
            elif session.event.group_id == 643815033:
                session.state['res'] = _cq_at_parser(session.event.user_id) + '你没有今日CP，快和Kiara贴贴去吧！'
            elif session.event.group_id == 826872326:
                session.state['res'] = _cq_at_parser(session.event.user_id) + '你没有今日CP，快和Calliope贴贴去吧！'
            else:
                session.state['res'] = _cq_at_parser(session.event.user_id) + '你没有今日CP，快和VTB贴贴去吧！'
        else:
            session.state['res'] = _cq_at_parser(session.event.user_id) + '你的今日CP是：' + _cq_at_parser(group_member_list[user_couple]['user_id']) + '，贴贴~'
        return
