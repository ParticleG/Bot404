...
parser.add_option("-d", "--splitsize", dest="splitsize",action="store",help="split to multiple file with size",default="30")#Minutes
...
for root,dirs,files in os.walk(options.srcD):
    for name in files:
        ...
        cmd ='mkdir -p ffm;rm -f ffm/ffm.txt;ffmpeg -i '+options.srcD+'/' +name+ ' >& ffm/ffm.txt;pwd'
        ...
        for c in result:
            ...
            for i in range(int(duration / (int(options.splitsize)*60)) + 1):
                ...
                os.chdir(options.workdir)
                cmd = "touch ffm/output.log;ffmpeg -y -i "+options.srcD+"/"+name+" -codec: copy -ss "+str(i*int(options.splitsize)*60)+" -t "+_t+" "+options.targetD+"/"+options.prefix+newname+'_'+string.replace(('%'+str(nameLength)+'s')%str(i),' ','0')+"."+options.fileType + ' >& ffm/output.log'
                for i in cmd.split(';'):
                    print i
                    time.sleep(2)
                    t1=subprocess.Popen(i,shell=True)
